      ...
      <div className="max-w-7xl mx-auto pt-40 px-8 pb-20">
        {!user ? (
          <div className="max-w-2xl mx-auto text-center p-12 border-4 border-black bg-white shadow-[16px_16px_0px_0px_rgba(0,0,0,1)]">
            <ShieldCheck size={64} className="mx-auto mb-6 text-blue-600" />
            <h1 className="text-5xl font-black italic uppercase mb-4">Identity Required</h1>
            <p className="mb-8 font-medium">To access the Orbit Control Plane, please authenticate with GitHub.</p>
            <button onClick={() => supabase.auth.signInWithOAuth({ provider: 'github' })} className="bg-black text-white px-10 py-4 font-bold uppercase tracking-widest hover:scale-105 transition-all">Login with GitHub</button>
          </div>
        ) : (
          <>
            <div className="flex justify-between items-end mb-16 border-b-4 border-black pb-8">
              <h1 className="text-8xl font-black italic uppercase tracking-tighter leading-none">Your Fleet<span className="text-blue-600">.</span></h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              ...
              <div className="border-4 border-dashed border-black/10 flex flex-col items-center justify-center p-12 opacity-40 hover:opacity-100 transition-all cursor-pointer group">
...
function SocialLink({ href, icon, label }: any) {
  return (
    <a href={href} target="_blank" className="flex items-center gap-2 px-6 py-3 border-2 border-black font-bold uppercase text-xs shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all bg-white">
      {icon} {label}
    </a>
  );
}